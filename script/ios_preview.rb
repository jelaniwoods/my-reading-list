...
module IosPreview
  ...
  class Artifact
    ...
    def initialize(archive:, manifest:, expected_sha:)
      ...
      raise Error, "expected a .app.zip artifact: #{@archive}" unless @archive.end_with?(".app.zip")

      data = JSON.parse(File.read(@manifest))
      @source_sha = data.fetch("commit_sha")
      @digest = Digest::SHA256.file(@archive).hexdigest

      raise Error, "unsupported Simulator manifest format" unless data["format"] == "firstdraft-simulator-app-v1"
      raise Error, "artifact is for #{data["app_platform"].inspect}, not iphonesimulator" unless data["app_platform"] == "iphonesimulator"
      ...
      architectures = data.fetch("architectures", "").split
      unless architectures.include?("arm64") && architectures.include?("x86_64")
        raise Error, "artifact must contain arm64 and x86_64 Simulator slices"
      end
    rescue JSON::ParserError, KeyError => error
      raise Error, "invalid Simulator manifest: #{error.message}"
    end
  end
  ...
  class ProjectConfig
    ...
    def initialize(root:)
      @path = File.join(root, "config", "ios_preview.json")
      raise Error, "missing config/ios_preview.json" unless File.file?(@path)

      @data = JSON.parse(File.read(@path))
      raise Error, "config/ios_preview.json must contain a JSON object" unless @data.is_a?(Hash)
    rescue JSON::ParserError => error
      raise Error, "invalid config/ios_preview.json: #{error.message}"
    end

    def project_name
      @data["name"].to_s.strip.then { |name| name.empty? ? "iOS preview" : name }
    end

    def app_name
      "#{project_name} iOS preview"
    end
  ...
  class GitHubArtifact
    WORKFLOW = "ios-simulator-artifact.yml"
    ...
    BUILD_INPUTS = [
      "ios",
      "bin/ios",
      "bin/ios-simulator-artifact",
      ".github/workflows/ios-simulator-artifact.yml"
    ].freeze
    ...
    def fetch(expected_sha)
      ...
      destination = File.join(@root, "tmp", "ios-preview", "#{expected_sha}-#{run.fetch("databaseId")}")
      destination = Dir.mktmpdir("#{expected_sha}-", File.join(@root, "tmp", "ios-preview")) if File.exist?(destination)
      FileUtils.mkdir_p(destination)

      @runner.run!("gh", "run", "download", run.fetch("databaseId").to_s, "--dir", destination)
      archives = Dir.glob(File.join(destination, "**", "*.app.zip"))
      manifests = Dir.glob(File.join(destination, "**", "manifest.json"))
      raise Error, "downloaded workflow artifact did not contain exactly one .app.zip and manifest.json" unless archives.one? && manifests.one?
    ...
    def wait_for_run(expected_sha)
      ...
      unless run
        ...
        @out.puts "No reusable native artifact exists; starting the GitHub macOS workflow."
        dispatch!(branch)
        15.times do
          @sleeper.call(2)
          run = pending_or_successful_run(expected_sha)
    ...
    def dispatch!(branch)
      ...
      raise Error,
        ...
        "Open your repository on GitHub → Actions → iOS Simulator artifact → Run workflow, " \
  ...
  class RevylAdapter
    ...
    def authenticate!
      ...
      @apps = json!("revyl", "app", "list", "--platform", "ios", "--json").fetch("apps")
    end
    ...
    def ensure_no_active_session!
      ...
      raise Error, "a preview session already exists; use `bin/ios preview revyl status` or stop it before starting another"
    end
    ...
    def app_id
      ...
      raise Error, "multiple Revyl apps are named #{@config.app_name.inspect}; set app_id in config/ios_preview.json" if matches.length > 1

      value = if matches.one?
        matches.first.fetch("id")
      else
        created = json!("revyl", "app", "create", "--name", @config.app_name, "--platform", "ios", "--json")
        created["id"] || created.dig("app", "id") || raise(Error, "Revyl did not return the created app ID")
      end
      @config.store_app_id!(value)
      @out.puts "Saved the non-secret Revyl app ID in config/ios_preview.json."
      value
    end
    ...
    def upload(artifact, app_id)
      ...
      data = json!(
        ...
        "--platform", "ios", "--version", version, "--yes", "--json"
      )
    ...
    def start(build_version_id:, origin:, timeout:, open:)
      output = @runner.capture!(
        "revyl", "device", "start", "--platform", "ios", "--build-version-id", build_version_id,
    ...
    def require_compatible_version!
      ...
      raise Error,
        ...
        "Follow IOS_PREVIEW.md to install the tested version."
    end
  ...
  class CLI
    ...
    def run
      ...
      @err.puts "iOS preview error: #{error.message}"
      1
    end
    ...
    def start
      ...
      parser = OptionParser.new do |opts|
        opts.banner = "Usage: bin/ios preview revyl --server URL [options]"
        opts.on("--server URL", "Public HTTPS Rails preview origin") { |value| options[:server] = value }
        opts.on("--artifact PATH", "Use an existing .app.zip instead of downloading GitHub Actions") { |value| options[:artifact] = value }
      ...
      @out.puts "Preparing the Simulator artifact in Revyl..."
      ...
      @out.puts "When finished: bin/ios preview revyl stop (closing the browser does not stop the device)."
    end
    ...
    def stop
      ...
      OptionParser.new do |opts|
        opts.banner = "Usage: bin/ios preview revyl stop [--session INDEX]"
    ...
    def doctor
      ...
      checks << ["Simulator workflow", File.file?(File.join(@root, ".github", "workflows", GitHubArtifact::WORKFLOW)), GitHubArtifact::WORKFLOW]
    ...
    def revyl_version_check
      ...
      detail = if compatible
        ...
        "requires #{required}; found #{output.inspect}; follow IOS_PREVIEW.md to install it"
      end
    ...
    def help
      <<~TEXT
        Usage:
          bin/ios preview revyl --server URL [--artifact APP.app.zip] [--manifest manifest.json]
          bin/ios preview revyl status
          bin/ios preview revyl stop [--session INDEX]
        ...
        build with identical native inputs or starts ios-simulator-artifact.yml,
        ...
        config/ios_preview.json contains only the app name and non-secret app ID.
